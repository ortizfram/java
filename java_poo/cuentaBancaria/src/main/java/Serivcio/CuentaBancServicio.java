...
public class CuentaBancServicio {
    ...
    public void Ingresar(CuentaBancaria c1, float salgoActual) {
        ...
        c1.setSalgoActual(c1.getIngreso() + c1.getSalgoActual());
        System.out.println("Saldo de : " + c1.getSalgoActual());
    }

    public void Retirar(CuentaBancaria c1, float salgoActual, float egreso) {
        Scanner leer = new Scanner(System.in);
        System.out.println("Monto a RETIRAR -$$$");
        c1.setEgreso(leer.nextFloat());
        if (c1.getSalgoActual() > 0) {
            c1.setSalgoActual(c1.getSalgoActual() - c1.getEgreso());
            System.out.println("Saldo de : " + c1.getSalgoActual());
        } else {
            System.out.println("**imposible : Saldo = 0");
        }
    }
     ...
     * más del 20%.
     */
    public void ExtraccionRapida(CuentaBancaria c1, float interes, float salgoActual) {
        /**
        ...
        if (c1.getInteres() <= 20) {
            ...
            if (c1.getSalgoActual() > 0) {
                c1.setSalgoActual(c1.getSalgoActual() - c1.getEgreso());
                System.out.println("Saldo de : " + (c1.getSalgoActual() - c1.getInteres()));
            } else {
                System.out.println("**imposible : Saldo = 0");
            }
        }
    ...
    public void mostrarSaldo(CuentaBancaria c1, float salgoActual) {
        System.out.println("Su saldo es de : " + c1.getSalgoActual());
    }
    ...
    public void mostrarDatos(CuentaBancaria c1, int DNI, int numeroCuenta, float salgoActual) {
        ...
        System.out.println("Saldo : " + c1.getSalgoActual());
    }
